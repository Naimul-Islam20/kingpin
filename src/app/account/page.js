    ...
    >
      <div className="flex items-center justify-between gap-3">
        <p className="text-sm font-bold text-gray-900">
          {SERVICE_CONFIG[item.type].label}
        </p>
        <span
          className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase ${STATUS_STYLES[item.status]}`}
        >
          {item.status}
        </span>
      </div>
      ...
      <span className="mt-3 inline-block text-[10px] font-black uppercase tracking-widest text-primary">
        View details →
      </span>
    </motion.button>
  );
...
function AccountPageContent() {
  ...
  const { member, pendingApplication, renewAnnualMembership, patchMember } =
    useDemoCustomer();

  const [activeTab, setActiveTab] = useState(
    queryTab && TABS.includes(queryTab) ? queryTab : "Overview",
  );
  ...
  const rewardPhase = useMemo(
    () => getRewardCardUiPhase(member, pendingApplication),
    [member, pendingApplication],
  );
  ...
  const upcoming = useMemo(
    () =>
      MOCK_BOOKINGS.find((item) => item.status === "Confirmed") ??
      MOCK_BOOKINGS[0],
    [],
  );

  const pendingBookings = useMemo(
    () => MOCK_BOOKINGS.filter((item) => item.status === "Pending").length,
    [],
  );

  const safeRedeem = Number(redeemPoints || 0);
  const maxRedeemByBalance = member.points;
  const maxRedeemByPolicy = Math.floor(
    upcoming.amountBDT * REDEEM_POLICY.maxRedeemPercentPerBooking,
  );
  const canRedeem =
  ...
  const handleRenewNow = () => {
    ...
    setMemberNotice(
      ok
        ? "Membership term extended. New expiry is shown above."
        : "Renewal is only available for active cards with a valid expiry date.",
    );
  };
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="max-w-lg text-sm text-gray-600">
                Bookings, points, and reward card — same look as the rest of
                Kingpin. Sample bookings below show how your hub will read once
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
              <Link
                href="/booking"
                className="inline-flex justify-center border-2 border-primary bg-white px-6 py-3 text-center text-[10px] font-black uppercase tracking-widest text-black transition hover:bg-gray-50"
              >
                New booking
              </Link>
              <Link
        ...
        <section className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            },
            {
              ...
              sub: `Redeem from ${REDEEM_POLICY.minimumPoints}+`,
            },
            {
            ...
            {
              label: "Expiring soon",
              value: String(member.pointsExpiringSoon),
              sub: `${REDEEM_POLICY.expiryWarningDays} day notice`,
            },
          ].map((card) => (
            <div
              key={card.label}
          ...
          <section className="min-h-[420px] border border-gray-100 bg-white p-6 shadow-sm md:p-8">
            {activeTab === "Overview" && (
              <>
                ...
                <p className="mt-1 text-sm text-gray-600">
                  Quick snapshot of your Kingpin profile.
                </p>
                ...
                {fromRegistration && (
                  <div className="mt-4 border border-[#C27D2A] bg-amber-50 p-4 text-sm text-amber-950">
                    <p className="font-black uppercase tracking-wide text-[#C27D2A]">
                      Account ready
                    </p>
                    <p className="mt-2">
                      Choose your next step — reward card or a reservation.
                    </p>
                    <div className="mt-3 flex flex-wrap gap-2">
                      <Link
                        href={membershipHref}
                ...
                {member.membershipStatus === MEMBERSHIP_STATUS.guest &&
                  rewardPhase.key !== "awaiting_account" && (
                    <div className="mt-4 border border-gray-200 bg-gray-50 p-4 text-sm text-gray-800">
                      <p className="font-bold text-gray-900">
                        No active reward card
                      </p>
                      <p className="mt-1 text-xs text-gray-600">
                        Apply once to earn and redeem points on qualifying
                        bookings.
                      </p>
                      <Link
                        href={membershipHref}
            ...
            {activeTab === "Rewards & Loyalty" && (
              <>
                ...
                <p className="mt-1 text-sm text-gray-600">
                  Programme rules for points and redemption.
                </p>
                <div className="mt-4 border border-gray-200 bg-gray-50 p-4 text-sm text-gray-800">
                  <p>Minimum redeem: {REDEEM_POLICY.minimumPoints} pts</p>
                  <p>
                    Max per booking:{" "}
                    {Math.round(REDEEM_POLICY.maxRedeemPercentPerBooking * 100)}
                    % of bill
                  </p>
                  <p>
                    Points expire after {REDEEM_POLICY.pointsExpiryMonths}{" "}
                    months of inactivity (policy).
                  </p>
                  {member.membershipStatus !== MEMBERSHIP_STATUS.active && (
                    <Link
                      href={membershipHref}
                ...
                <div className="mt-4 border border-gray-200 bg-white p-4 shadow-sm">
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    Sample booking amount: {formatBDT(upcoming.amountBDT)}
                  </p>
                  {member.membershipStatus !== MEMBERSHIP_STATUS.active && (
                  ...
                  />
                  <p
                    className={`mt-2 text-xs font-semibold ${canRedeem ? "text-emerald-700" : "text-amber-800"}`}
                  >
                    {canRedeem
                      ? "Within programme limits for this booking amount."
                      : "Not redeemable — check card status, balance, and cap."}
                  </p>
                </div>
                <div className="mt-4 space-y-2">
                  {ledgerRows.map((entry) => (
                    <div
                      key={entry.id}
                      className="border border-gray-100 bg-gray-50 p-3 text-sm"
                    >
                      <p className="font-bold text-gray-900">
                        {entry.event}{" "}
            ...
            {activeTab === "Membership" && (
              <>
                ...
                <p className="mt-1 text-sm text-gray-600">
                  Apply → verify → pay → link account. Your card activates as
                  soon as payment is matched to your profile.
                </p>
                ...
                <div className="mt-4 grid gap-3 border border-gray-200 bg-gray-50 p-4 text-sm sm:grid-cols-2">
                  ...
                  <p>
                    ...
                    <span className="font-bold text-gray-900">
                      ...
                      {formatBDT(
                        member.annualFeeBDT ?? MEMBERSHIP_PLAN.annualFeeBDT,
                      )}
                      )
                    </span>
                  </p>
                  <p>
                    ...
                    <br />
                    <span className="font-bold text-gray-900">
                      {member.verification?.phoneVerified
                        ? "Verified"
                        : "Not verified"}
                    </span>
                  </p>
                    ...
                    <>
                      <p className="sm:col-span-2">
                        ...
                        <span className="font-mono text-base font-bold tracking-wide text-gray-900">
                          {member.rewardCardPan || "Issuing…"}
                        </span>
                      </p>
                      <p className="sm:col-span-2 text-xs text-gray-600">
                        Valid {member.membershipStartDate || "—"} —{" "}
                        {member.membershipExpiryDate || "—"}
                        {member.cardActivatedAt
                          ? ` · Issued ${String(member.cardActivatedAt).slice(0, 10)}`
                          : ""}
                      </p>
                    </>
                  )}
            ...
            {activeTab === "Profile" && (
              <>
                ...
                <p className="mt-1 text-sm text-gray-600">
                  Update how we address you. This build stores changes in your
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-gray-700">
              Type: {SERVICE_CONFIG[selectedBooking.type].label}
            </p>
            <p className="text-sm text-gray-700">
              When: {selectedBooking.dateTime}
            </p>
            <p className="text-sm text-gray-700">
