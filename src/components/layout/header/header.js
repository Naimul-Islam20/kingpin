...
export default function Header() {
  ...
  <div className="max-w-[1250px] mx-auto flex justify-between items-center px-4 md:px-12 lg:px-16 py-4">
    {/* Left: Logo */}
          ...
          {menus.map((menu) => (
            ...
            >
              ...
              <Link
                ...
                className="flex items-center gap-1 px-2 py-2 text-gray-700 hover:text-blue-600 transition-colors duration-200"
              >
                {menu.menu_name}
                {menu.has_child === 1 && (
              ...
              {menu.children && openMenu === menu.id && (
                <>
                  ...
                  <div
                    className="absolute left-0 top-full mt-4 w-52 bg-white border-gray-200 border rounded shadow-md"
                    onMouseEnter={() => setOpenMenu(menu.id)}
                    onMouseLeave={() => setOpenMenu(null)}
                  >
                    <ul className="py-2">
                      {menu.children.map((child) => (
                        ...
                        >
                          <Link
                            href={child.path && child.path !== "" ? child.path : "#"}
                            className="flex justify-between items-center w-full px-4 py-2 hover:bg-gray-100 transition"
                          >
                            {child.menu_name}
                            {child.has_child === 1 && (
                            ...
                            <>
                              ...
                              >
                                {child.children.map((sub) => (
                                  <li key={sub.id}>
                                    <Link
                                      href={sub.path && sub.path !== "" ? sub.path : "#"}
